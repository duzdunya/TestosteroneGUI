SRCPATH := src
OBJS := animations calculations colors dstructures easings elements initial rendering testo testo_events run
OBJS_T := test

CC = gcc
CFLAGS = -lm -Wall -Iheaders -lSDL2_image -lSDL2_ttf -I/usr/include/fontconfig -lfontconfig $(shell sdl2-config --cflags --libs)

.PHONY := run test objects clear
...
objects: $(addsuffix .o, $(addprefix $(SRCPATH)/,$(OBJS)))
